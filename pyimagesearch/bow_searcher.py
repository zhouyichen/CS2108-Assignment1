...
def search_for_match(image_ids, im_features, test_features):
    results = {}
    for i in range(len(im_features)):
        d = chi2_distance(im_features[i], test_features)
        results[image_ids[i]] = d
...
class BOW_Searcher:
	...
	def search(self, query_image, limit = 20):
		...
		image_ids, im_features, k, voc = joblib.load(self.sift_bow_data)

		test_features = np.zeros(k, "float32")

		words, distance = vq(query_features, voc)
		for w in words:
			test_features[w] += 1
		test_features = cv2.normalize(test_features).T

		results = search_for_match(image_ids, im_features, test_features)
